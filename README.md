...
- **只覆盖要改的色值，然后深合并。** 地图数据里只写差异，"整块替换"的话 `BASE_THEME` 里将来新增的字段在被覆盖的地图上会变成 `undefined` —— 那种崩溃**只在切到那张地图时**出现，是最难查的一类。`deepMerge()` 之后，测试 L 会逐字段检查每张地图的主题没有缺项。
...
node dev/e2e-test.js        # 305 项断言，含 UI 契约、拖动 1:1、齐射弹道、移动端视口、路沿透视、金币经济、商城
...
- **每张地图的每种景物都要登记"最坏横向半宽"**：`test E` 拿这张表去算"物件离路面还有多少米"，
  查不到的名字按 0 算——也就是说**漏登一个，它就等于被放行去压路面**。沙漠图那 7 种物件是补进去的，
